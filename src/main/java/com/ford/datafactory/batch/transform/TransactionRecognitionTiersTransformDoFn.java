...
import com.ford.datafactory.batch.decoder.TransactionRecognitionTiersDecoder;
...
public class TransactionRecognitionTiersTransformDoFn extends DoFn<FileIO.ReadableFile , String> {
    private static final Logger logger = Logger.getLogger(TransactionRecognitionTiersTransformDoFn.class.getName());
...
// We can now access the file and its metadata.
        ...
        if (fileName.contains("TransactionRecognitionTiers") &&
            ...
            try (InputStream stream = Channels.newInputStream(file.open())) {
                ...
                while ((line = streamReader.readLine()) != null) {
                    ...
                    String record = new TransactionRecognitionTiersDecoder().decode(headerTimestamp, line);
                    if (null != record) {
