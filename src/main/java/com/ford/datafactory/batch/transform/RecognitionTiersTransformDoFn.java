...
import com.ford.datafactory.batch.decoder.RecognitionTiersDecoder;
...
public class RecognitionTiersTransformDoFn extends DoFn<FileIO.ReadableFile , String> {
    private static final Logger logger = Logger.getLogger(RecognitionTiersTransformDoFn.class.getName());
    @ProcessElement
...
// We can now access the file and its metadata.
        ...
        try (InputStream stream = Channels.newInputStream(file.open())) {
            ...
            while ((line = streamReader.readLine()) != null) {
                ...
                String record = new RecognitionTiersDecoder().decode(file.getMetadata().resourceId().getFilename().toString(),line);
                if(null != record) {
