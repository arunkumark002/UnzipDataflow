...
public class TransactionPointsBalancesTransformDoFn extends DoFn<FileIO.ReadableFile , String> {
    private static final Logger logger = Logger.getLogger(TransactionPointsBalancesTransformDoFn.class.getName());
...
// We can now access the file and its metadata.
        ...
        if (fileName.contains("TransactionPointsBalances") &&
            ...
            try (InputStream stream = Channels.newInputStream(file.open())) {
                ...
                while ((line = streamReader.readLine()) != null) {
                    ...
                    String record = new TransactionPointsBalancesDataDecoder().decode(headerTimestamp, line);
                    if (null != record) {
