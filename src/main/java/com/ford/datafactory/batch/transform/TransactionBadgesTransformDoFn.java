...
import com.ford.datafactory.batch.decoder.TransactionBadgesDataDecoder;
...
public class TransactionBadgesTransformDoFn extends DoFn<FileIO.ReadableFile , String> {
    private static final Logger logger = Logger.getLogger(TransactionBadgesTransformDoFn.class.getName());
    @ProcessElement
...
// We can now access the file and its metadata.
        ...
        if(fileName.contains("TransactionBadges") &&
            ...
            try (InputStream stream = Channels.newInputStream(file.open())) {
                ...
                while ((line = streamReader.readLine()) != null) {
                    ...
                    String record = new TransactionBadgesDataDecoder().decodeWithCntry(headerTimestamp, line, countryCode);
                    if (null != record) {
