...
import com.ford.datafactory.batch.decoder.TransactionBasketItemRedemptionDataDecoder;
...
public class TransactionBasketItemRedemptionTransformDoFn extends DoFn<FileIO.ReadableFile , String> {
    private static final Logger logger = Logger.getLogger(TransactionBasketItemRedemptionTransformDoFn.class.getName());
...
// We can now access the file and its metadata.
        ...
        if (fileName.contains("TransactionBasketItemRedemption") &&
            ...
            try (InputStream stream = Channels.newInputStream(file.open())) {
                ...
                while ((line = streamReader.readLine()) != null) {
                    ...
                    String record = new TransactionBasketItemRedemptionDataDecoder().decode(headerTimestamp, line);
                    if (null != record) {
