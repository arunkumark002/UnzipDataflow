...
import com.ford.datafactory.batch.decoder.TransactionCouponRedemptionDataDecoder;
...
public class TransactionCouponRedemptionTransformDoFn extends DoFn<FileIO.ReadableFile , String> {
    private static final Logger logger = Logger.getLogger(TransactionCouponRedemptionTransformDoFn.class.getName());
...
// We can now access the file and its metadata.
        ...
        if (fileName.contains("TransactionCouponRedemption") &&
            ...
            try (InputStream stream = Channels.newInputStream(file.open())) {
                ...
                while ((line = streamReader.readLine()) != null) {
                    ...
                    String record = new TransactionCouponRedemptionDataDecoder().decode(headerTimestamp, line);
                    if (null != record) {
