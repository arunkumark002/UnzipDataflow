...
import com.ford.datafactory.batch.decoder.TransactionPromotionCountersDataDecoder;
import com.ford.datafactory.batch.decoder.TransactionPromotionPointsDataDecoder;
...
public class TransactionPromotionCountersTransformDoFn extends DoFn<FileIO.ReadableFile , String> {
    private static final Logger logger = Logger.getLogger(TransactionRuleTransformDoFn.class.getName());
...
// We can now access the file and its metadata.
        ...
        if (fileName.contains("TransactionPromotionCounters") &&
            ...
            try (InputStream stream = Channels.newInputStream(file.open())) {
                ...
                while ((line = streamReader.readLine()) != null) {
                    ...
                    String record = new TransactionPromotionCountersDataDecoder().decode(headerTimestamp, line);
                    if (null != record) {
