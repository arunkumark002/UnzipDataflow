...
import com.ford.datafactory.batch.decoder.TransactionPointsAggregateDataDecoder;
...
public class TransactionPointsAggregateTransformDoFn extends DoFn<FileIO.ReadableFile , String> {
    private static final Logger logger = Logger.getLogger(TransactionPointsAggregateTransformDoFn.class.getName());
...
// We can now access the file and its metadata.
        ...
        if (fileName.contains("TransactionPointsAggregate") &&
            ...
            try (InputStream stream = Channels.newInputStream(file.open())) {
                ...
                while ((line = streamReader.readLine()) != null) {
                    ...
                    String record = new TransactionPointsAggregateDataDecoder().decode(headerTimestamp, line);
                    if (null != record) {
