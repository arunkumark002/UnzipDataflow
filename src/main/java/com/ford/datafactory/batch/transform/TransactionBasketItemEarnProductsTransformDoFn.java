...
import com.ford.datafactory.batch.decoder.TransactionBasketItemEarnProductsDataDecoder;
...
public class TransactionBasketItemEarnProductsTransformDoFn extends DoFn<FileIO.ReadableFile , String> {
    private static final Logger logger = Logger.getLogger(TransactionBasketItemEarnProductsTransformDoFn.class.getName());
...
// We can now access the file and its metadata.
        ...
        if (fileName.contains("TransactionBasketItemEarnProducts") &&
            ...
            try (InputStream stream = Channels.newInputStream(file.open())) {
                ...
                while ((line = streamReader.readLine()) != null) {
                    ...
                    String record = new TransactionBasketItemEarnProductsDataDecoder().decode(headerTimestamp, line);
                    if (null != record) {
