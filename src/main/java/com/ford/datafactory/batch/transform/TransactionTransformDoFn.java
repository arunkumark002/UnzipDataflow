...
import com.ford.datafactory.batch.decoder.TransactionDataDecoder;
...
public class TransactionTransformDoFn extends DoFn<FileIO.ReadableFile , String> {
    private static final Logger logger = Logger.getLogger(TransactionTransformDoFn.class.getName());
...
// We can now access the file and its metadata.
        ...
        if (fileName.contains("Transaction_") &&
            ...
            try (InputStream stream = Channels.newInputStream(file.open())) {
                ...
                while ((line = streamReader.readLine()) != null) {
                    ...
                    String record = new TransactionDataDecoder().decode(headerTimestamp, line);
                    if (null != record) {
